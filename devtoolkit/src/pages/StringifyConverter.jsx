...
import { ArrowRight, ArrowLeft, ArrowLeftRight, Copy, Download, Upload, Trash2, Check, ClipboardPaste, AlertCircle } from "lucide-react";
...
export default function StringifyConverter() {
  ...
  const handleStringify = () => {
    if (!jsonInput.trim()) {
      addToast({ title: "No input", description: "Paste or type JSON first.", type: "error" });
      return;
    }
    try {
  ...
  const handleParse = () => {
    if (!stringInput.trim()) {
      addToast({ title: "No input", description: "Paste a stringified JSON first.", type: "error" });
      return;
    }
    try {
    ...
    return (
      <div className="max-w-screen-xl mx-auto px-4 py-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div>
            <h1 className="text-xl font-bold mb-1">Stringify ↔ JSON — Live Sync</h1>
            <p className="text-base text-muted-foreground">Edit either panel and the other updates automatically (300ms debounce).</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold">Normal JSON (pretty)</span>
              {liveJsonError && (
                <span className="flex items-center gap-1 text-xs text-red-400">
                  <AlertCircle className="h-3 w-3" /> {liveJsonError.slice(0, 40)}
                </span>
              )}
            </div>
            <textarea
              ...
              onChange={e => {
                ...
                debounceRef.current = setTimeout(() => runLiveStringify(e.target.value), 300);
              }}
              className={cn(
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold">Stringified (escaped string)</span>
              {liveStringError && (
                <span className="flex items-center gap-1 text-xs text-red-400">
                  <AlertCircle className="h-3 w-3" /> {liveStringError.slice(0, 40)}
                </span>
              )}
            </div>
            <textarea
              ...
              onChange={e => {
                ...
                clearTimeout(debounceRef.current);
                debounceRef.current = setTimeout(() => runLiveParse(e.target.value), 300);
              }}
              className={cn(
                "w-full min-h-[440px] p-4 rounded-lg border bg-card font-mono text-sm leading-relaxed resize-y focus:outline-none focus:ring-1",
                liveStringError ? "border-red-500/50 focus:ring-red-500/30" : liveStringified ? "border-blue-500/40 focus:ring-blue-500/30" : "border-border"
              )}
            />
          </div>
        </div>
        ...
        <p className="mt-4 text-sm text-muted-foreground">
          <strong>Tip:</strong> Left shows your JSON formatted. Right shows it as an escaped string literal (e.g. <code className="font-mono text-xs bg-muted px-1 py-0.5 rounded">{'"{\\"key\\":\\"val\\"}"'}</code>) ready to embed in code.
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-screen-xl mx-auto px-4 py-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-base text-muted-foreground">Convert between JSON objects and escaped string literals in both directions.</p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            onClick={() => { setLiveMode(true); runLiveStringify(liveJson); }}
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full">Object → Escaped String</span>
          </div>

          <p className="text-sm text-muted-foreground">
            Takes your JSON object and converts it to an escaped string literal — exactly what <code className="font-mono text-xs bg-muted px-1 rounded">JSON.stringify(obj)</code> produces wrapped in quotes.
          </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground font-medium">Input — Normal JSON</span>
                <div className="flex gap-1">
                  <button onClick={async () => { setJsonInput(await navigator.clipboard.readText()); setStringifiedOutput(""); setJsonifyError(null); }} className="flex items-center gap-1 px-2 py-1 text-sm text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors">
                    <ClipboardPaste className="h-3.5 w-3.5" /> Paste
                  </button>
                  <button onClick={() => fileRefA.current?.click()} className="flex items-center gap-1 px-2 py-1 text-sm text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors">
                    <Upload className="h-3.5 w-3.5" /> Upload
                  </button>
                  <button onClick={() => { setJsonInput(""); setStringifiedOutput(""); setJsonifyError(null); }} className="flex items-center gap-1 px-2 py-1 text-sm text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors">
                    <Trash2 className="h-3.5 w-3.5" />
                  </button>
                  <input ref={fileRefA} type="file" accept=".json,.txt" className="hidden" onChange={e => {
                    const f = e.target.files[0]; if (!f) return;
                    const r = new FileReader(); r.onload = ev => { setJsonInput(ev.target.result); setStringifiedOutput(""); setJsonifyError(null); }; r.readAsText(f); e.target.value = "";
                  }} />
                </div>
              </div>
              <textarea
                value={jsonInput}
                onChange={e => { setJsonInput(e.target.value); setStringifiedOutput(""); setJsonifyError(null); }}
                placeholder={'{\n  "name": "Alice",\n  "age": 30\n}'}
                className={cn(
                  ...
                  jsonifyError ? "border-red-500/50 focus:ring-red-500/30" : "border-border focus:ring-ring/30"
                )}
              />
              {jsonifyError && (
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground font-medium">Output — Stringified</span>
                <div className="flex gap-1">
                  {stringifiedOutput && <CopyBtn text={stringifiedOutput} />}
                  {stringifiedOutput && <DownloadBtn text={stringifiedOutput} filename="stringified.txt" label=".txt" />}
                </div>
              </div>
              <div className={cn(
                ...
                {stringifiedOutput
                  ? <span className="text-blue-400 dark:text-blue-300">{stringifiedOutput}</span>
                  : <span className="text-muted-foreground">Stringified result will appear here…<br /><br />Example output:<br /><span className="text-blue-400">{'"{\\"name\\":\\"Alice\\",\\"age\\":30}"'}</span></span>
                }
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            {stringifiedOutput && (
              <button
                onClick={() => { setStringInput(stringifiedOutput); setJsonOutput(""); setParseError(null); }}
        ...
        <div className="rounded-xl border border-border bg-card p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-muted-foreground bg-muted px-2 py-0.5 rounded-full">Escaped String → Object</span>
          </div>

          <p className="text-sm text-muted-foreground">
            Paste a stringified JSON string (with or without surrounding quotes). It will be unescaped and parsed back to a readable JSON object.
          </p>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground font-medium">Input — Stringified</span>
                <div className="flex gap-1">
                  <button onClick={async () => { setStringInput(await navigator.clipboard.readText()); setJsonOutput(""); setParseError(null); }} className="flex items-center gap-1 px-2 py-1 text-sm text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors">
                    <ClipboardPaste className="h-3.5 w-3.5" /> Paste
                  </button>
                  <button onClick={() => fileRefB.current?.click()} className="flex items-center gap-1 px-2 py-1 text-sm text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors">
                    <Upload className="h-3.5 w-3.5" /> Upload
                  </button>
                  <button onClick={() => { setStringInput(""); setJsonOutput(""); setParseError(null); }} className="flex items-center gap-1 px-2 py-1 text-sm text-muted-foreground hover:text-foreground hover:bg-accent rounded transition-colors">
                    <Trash2 className="h-3.5 w-3.5" />
                  </button>
                  <input ref={fileRefB} type="file" accept=".json,.txt" className="hidden" onChange={e => {
                    const f = e.target.files[0]; if (!f) return;
                    const r = new FileReader(); r.onload = ev => { setStringInput(ev.target.result); setJsonOutput(""); setParseError(null); }; r.readAsText(f); e.target.value = "";
                  }} />
                </div>
              </div>
              <textarea
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground font-medium">Output — Parsed JSON</span>
                <div className="flex gap-1">
                  {jsonOutput && <CopyBtn text={jsonOutput} />}
                  {jsonOutput && <DownloadBtn text={jsonOutput} filename="parsed.json" label=".json" />}
                </div>
              </div>
              <div className={cn(
                ...
                {jsonOutput
                  ...
                  : <span className="text-muted-foreground">Parsed JSON will appear here…</span>
                }
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            {jsonOutput && (
              <button
                onClick={() => { setJsonInput(jsonOutput); setStringifiedOutput(""); setJsonifyError(null); }}
                className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-border text-sm font-medium hover:bg-accent transition-colors"
              >
