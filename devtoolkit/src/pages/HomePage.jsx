    ...
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden flex flex-col items-center justify-center min-h-[calc(100vh-56px)] px-4 py-20">
        ...
        {FLOATING_SNIPPETS.map((s, i) => (
          <span
            key={i}
            className={cn("floating-code", devMode ? "text-green-400" : "text-foreground/50 dark:text-foreground/40")}
            style={{
              left: s.x,
              bottom: "-80px",
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center">
          {devMode && (
            <div className="font-mono text-green-400 text-sm mb-4 opacity-80">
              &gt; initializing DevToolkit v1.0.0 ... [OK]<br />
          ...
          >
            ...
            <br />
            {devMode ? "  You Need._" : <>You Need.<span className="cursor-blink ml-1 text-muted-foreground">|</span></>}
          </h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-14">
            {STATS.map(({ icon: Icon, label }) => (
              ...
              >
                <Icon className={cn("h-5 w-5", devMode ? "text-green-400" : "text-muted-foreground")} />
                <span className={cn("text-sm font-medium", devMode && "text-green-300 font-mono")}>{label}</span>
              </div>
            ))}
          </div>
        ...
        {/* Scroll indicator */}
        <div className={cn("absolute bottom-8 left-1/2 -translate-x-1/2 bounce-down", devMode ? "text-green-400" : "text-muted-foreground")}>
      ...
      <section id="features-section" className="px-4 py-20 max-w-screen-xl mx-auto">
        <div className="text-center mb-12">
          <h2 className={cn("text-2xl sm:text-3xl font-bold mb-3", devMode && "text-green-400 font-mono")}>
            {devMode ? "> Everything you need, nothing you don't." : "Everything you need, nothing you don't."}
          </h2>
          <p className="text-muted-foreground text-base">Four powerful tools, zero dependencies, all in your browser.</p>
        </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  devMode
                    ...
                    : cn("border bg-card", tool.border, `hover:shadow-${tool.glow}`)
                )}
              >
                {/* Gradient background */}
                {!devMode && (
                  <div className={cn("absolute inset-0 bg-gradient-to-br opacity-0 group-hover:opacity-100 transition-opacity duration-300", tool.gradient)} />
                )}
                {devMode && (
                ...
                <div className={cn(
                  ...
                  devMode ? "bg-gradient-to-r from-transparent via-green-400/60 to-transparent" : `bg-gradient-to-r from-transparent via-current to-transparent ${tool.color}`
                )} />
                  ...
                  <div className="flex items-center justify-between mb-5">
                    <div className={cn(
                      ...
                      devMode
                        ...
                        : cn(tool.iconBg, "border-border group-hover:scale-110")
                    )}>
                      <Icon className={cn("h-5 w-5 transition-all", devMode ? "text-green-400" : tool.color)} />
                    </div>
                    <span className={cn(
                      ...
                      devMode ? "border-green-500/40 text-green-400/80 bg-green-500/5 font-mono" : cn("border-border text-muted-foreground", tool.color.replace("text-", "text-"))
                    )}>
                      {tool.badge}
                    </span>
                  </div>

                  <h3 className={cn("font-semibold text-base mb-2 transition-colors", devMode && "text-green-300 font-mono group-hover:text-green-400")}>
                    {tool.name}
                  </h3>
...
function JsonHighlight({ json }) {
  ...
  while ((match = re.exec(json)) !== null) {
    if (match.index > lastIndex) tokens.push(<span key={k++}>{json.slice(lastIndex, match.index)}</span>);
    const val = match[0];
    if (val.endsWith(":")) tokens.push(<span key={k++} className="text-blue-400">{val}</span>);
    else if (val.startsWith('"')) tokens.push(<span key={k++} className="text-green-400">{val}</span>);
    else if (match[2] !== undefined) tokens.push(<span key={k++} className="text-orange-400">{val}</span>);
    else if (val === "true" || val === "false") tokens.push(<span key={k++} className="text-purple-400">{val}</span>);
    else if (val === "null") tokens.push(<span key={k++} className="text-red-400">{val}</span>);
    else tokens.push(<span key={k++}>{val}</span>);
    lastIndex = re.lastIndex;
  }
  if (lastIndex < json.length) tokens.push(<span key={k++}>{json.slice(lastIndex)}</span>);
  return <>{tokens}</>;
}
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        ...
        devMode ? "border-green-500/20 bg-green-500/5" : "border-border bg-background/50"
      )}>
        <div className="w-2.5 h-2.5 rounded-full bg-red-500/60" />
        <div className="w-2.5 h-2.5 rounded-full bg-yellow-500/60" />
        <div className="w-2.5 h-2.5 rounded-full bg-green-500/60" />
        <span className={cn("ml-2 text-xs font-mono", devMode ? "text-green-400/70" : "text-muted-foreground")}>
          json-formatter.js
        </span>
      </div>
      <div className="grid grid-cols-2 divide-x divide-border">
        <div className="p-4">
          <div className={cn("text-xs mb-2 font-medium font-mono", devMode ? "text-green-400/60" : "text-muted-foreground")}>Input</div>
          <pre className={cn("text-xs font-mono leading-relaxed overflow-hidden whitespace-pre-wrap break-all", devMode ? "text-red-400/70" : "text-red-400/80")}>
            {UGLY_JSON}
          </pre>
        </div>
        <div className="p-4">
          <div className={cn("text-xs mb-2 font-medium font-mono", devMode ? "text-green-400/60" : "text-muted-foreground")}>Formatted ✓</div>
